...
const AdminDashboard: React.FC = () => {
  ...
  return (
    ...
    >
      <div
        className="p-8 border-b border-white/5 bg-white/5 flex flex-col xl:flex-row justify-between items-center gap-6">
          ...
          <>
            {activeTab === 'proofs' && (
              <div className="space-y-4">
                <p className="text-zinc-500 text-[10px] font-black uppercase tracking-[0.3em] mb-6">Черга модерації: 14
                  активних заявок</p>
                {[
                  {
                    id: '1',
                  ...
                  <div key={item.id}
                    ...
                    <div className="mb-6 lg:mb-0">
                      <div className="flex items-center space-x-2 mb-2">
                        <p
                          className="text-[10px] text-emerald-500 font-black uppercase tracking-widest">Об'єкт: {item.target}</p>
                        <span
                          className="text-[9px] text-zinc-600 px-1.5 py-0.5 rounded bg-white/5 border border-white/5">SUBMITTER: #{item.authorId}</span>
                      </div>
                      <p className="text-base text-zinc-200 font-medium leading-relaxed max-w-md">"{item.text}"</p>
                      <p
                        className="text-[9px] text-zinc-600 mt-3 font-bold uppercase tracking-widest italic">Джерело: {item.source} • {item.time}</p>
                    </div>
            ...
            {activeTab === 'entities' && (
              <div className="overflow-x-auto">
                <div className="flex justify-between items-center mb-6">
                  <p className="text-zinc-500 text-[10px] font-black uppercase tracking-[0.3em]">Реєстр публічних
                    осіб</p>
                  <button
                    className="px-4 py-2 bg-emerald-500 text-zinc-950 rounded-xl text-[10px] font-black uppercase tracking-widest">+
                    Додати особу
                  </button>
                </div>
                <table className="w-full text-left border-collapse">
                  <thead>
                  <tr
                    className="text-zinc-500 text-[10px] font-black uppercase tracking-[0.2em] border-b border-white/5">
            ...
            {activeTab === 'users' && (
              <div className="overflow-x-auto">
                <p className="text-zinc-500 text-[10px] font-black uppercase tracking-[0.3em] mb-6">База зареєстрованих
                  користувачів</p>
                <table className="w-full text-left border-collapse">
                  <thead>
                  <tr
            ...
            {activeTab === 'audit' && (
              <div className="space-y-4">
                <p className="text-zinc-500 text-[10px] font-black uppercase tracking-[0.3em] mb-6">Журнал дій
                  адміністрації</p>
                {logs.map(log => (
                  <div key={log.id}
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        <p className="text-xs font-bold"><span
                          className="text-emerald-500">{log.adminName}</span> — {log.action}</p>
                        <p
                          className="text-[10px] text-zinc-500 uppercase font-black tracking-widest mt-1">Ціль: {log.targetName}</p>
                      </div>
                    </div>
                    <span
                      className="text-[10px] font-black text-zinc-600 uppercase tracking-widest">{log.timestamp}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {activeTab === 'ai' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="col-span-full mb-4">
                  <p className="text-zinc-500 text-[10px] font-black uppercase tracking-[0.3em]">Висновки
                    AI-Консиліуму</p>
                </div>
                {insights.map(insight => (
                  <div key={insight.id} className="glass p-6 rounded-3xl border-white/10 relative">
                    <div
                      className="absolute top-4 right-4 bg-emerald-500/10 text-emerald-500 text-[10px] font-black px-2 py-1 rounded-lg">
                    ...
                    <div className="flex items-start space-x-4">
                      ...
                      <div>
                        <h4
                          className="font-black tracking-tight mb-2">Об'єкт: {people.find(p => p.id === insight.targetId)?.name}</h4>
